"""Prepare a small, validated RGB-to-albedo pilot without downloading full datasets.

The downloader deliberately avoids Hugging Face ``datasets`` streaming for Olbedo,
which can touch multi-gigabyte Parquet shards before yielding examples. It uses the
Dataset Viewer row API for selected Olbedo rows and downloads only individually
selected files from SynCom and LightCity.

Default output: 96 aligned 512x512 pairs (32 per domain), with group-disjoint train
and validation manifests plus a contact sheet for visual inspection.
"""
...
USER_AGENT = "voir-fast-pilot/1.0"


def _request_json(url: str, token: str | None = None, retries: int = 8) -> dict[str, Any]:
...
def _request_bytes(url: str, token: str | None = None, retries: int = 8) -> bytes:
    headers = {"User-Agent": USER_AGENT}
    if token:
        headers["Authorization"] = f"Bearer {token}"
    last: Exception | None = None
...
def _syncom_descriptors(count: int, token: str | None, seed: int) -> list[dict[str, Any]]:
    ...
    for path in files:
        ...
        if key is not None:
            ...
            # Prefer PNG when available, otherwise retain the first numeric map.
...
def _lightcity_key(path: str, *, albedo: bool) -> tuple[str, str] | None:
    ...
    if albedo:
        marker = "_Albedo"
        if marker.lower() not in stem.lower():
            return None
        position = stem.lower().index(marker.lower())
...
def _lightcity_descriptors(count: int, token: str | None, seed: int) -> list[dict[str, Any]]:
    from huggingface_hub import HfApi

    repo = "whalejj/LightCity"
    print(f"Listing {repo} files...", flush=True)
    files = HfApi(token=token).list_repo_files(repo, repo_type="dataset")
    albedo_map: dict[tuple[str, str], str] = {}
    for path in files:
        key = _lightcity_key(path, albedo=True)
        if key is None:
            continue
        current = albedo_map.get(key)
        # Prefer the shortest canonical name if several postfix variants exist.
        if current is None or len(Path(path).name) < len(Path(current).name):
            albedo_map[key] = path
    candidates: list[dict[str, Any]] = []
    for path in files:
        key = _lightcity_key(path, albedo=False)
        if key is None or key not in albedo_map:
            continue
        parent, frame = key
        parts = [part for part in parent.split("/") if part]
        group = "/".join(parts[:3]) if len(parts) >= 3 else parent
        candidates.append(
            {
                "kind": "hub",
                "domain": "lightcity",
                "group": group,
                "source_id": f"{repo}:{parent}:{frame}",
                "repo": repo,
                "image_path": path,
                "albedo_path": albedo_map[key],
                "metadata": {
                    "source_dataset": repo,
                    "source_split": "intrinsic_pair",
                    "scene": parent,
                },
            }
        )
    if len(candidates) < count:
        raise RuntimeError(f"LightCity produced {len(candidates)} matched RGB/albedo files; requested {count}")
    return _balanced_take(candidates, count, seed)
...
def _load_descriptor(item: dict[str, Any], token: str | None) -> tuple[Image.Image, Image.Image, Image.Image | None]:
    ...
    albedo_path = hf_hub_download(
        repo_id=item["repo"],
        filename=item["albedo_path"],
        repo_type="dataset",
        token=token,
    )
    image = _decode_numeric(image_path, linear_to_srgb=Path(image_path).suffix.lower() in {".exr", ".hdr"})
    albedo = _decode_numeric(albedo_path, linear_to_srgb=Path(albedo_path).suffix.lower() in {".exr", ".hdr", ".npy"})
...
def _split_groups(items: list[dict[str, Any]], validation_fraction: float, seed: int) -> set[tuple[str, str]]:
    ...
    for domain, groups in by_domain.items():
        ranked = sorted(
            groups,
            key=lambda group: hashlib.sha256(f"{seed}|{domain}|{group}".encode()).digest(),
        )
...
def _contact_sheet(output: Path, rows: list[dict[str, Any]], limit: int = 12) -> Path:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare a fast multi-domain RGB/albedo pilot.")
    parser.add_argument("--output", default="/content/voir_pilot96")
    parser.add_argument("--size", type=int, default=512)
    parser.add_argument("--olbedo-count", type=int, default=32)
    parser.add_argument("--syncom-count", type=int, default=32)
    parser.add_argument("--lightcity-count", type=int, default=32)
    ...
    expected = args.olbedo_count + args.syncom_count + args.lightcity_count
    ...
    print("Collecting lightweight descriptors; no full repository snapshots will be downloaded.", flush=True)
    domains = {
        ...
        "lightcity": _lightcity_descriptors(args.lightcity_count, token, args.seed + 2),
    }
    ...
    (output / "pairs.partial.jsonl").unlink(missing_ok=True)
    sheet = _contact_sheet(output, records)

    domain_counts: dict[str, int] = defaultdict(int)
    split_counts: dict[str, int] = defaultdict(int)
    for row in records:
        domain_counts[row["domain"]] += 1
        split_counts[row["subset"]] += 1
    metadata = {
        "status": "PASS",
        ...
        "sources": {
            "GDAOSU/Olbedo": "CC BY 4.0; selected via Dataset Viewer row API",
            "yGaoJiany/SynCom": "CC BY 4.0; selected individual object RGB/albedo files",
            "whalejj/LightCity": "MIT; selected individual urban RGB/albedo files",
        },
    }
