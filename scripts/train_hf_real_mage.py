...
from voir.real_mage import (
    RealMageAlbedoNet,
    RealMageCacheDataset,
    compute_branch_normalization,
    cosine_warmup,
)
...
def _resolve_cache_root(dataset_dir: Path) -> Path:
    candidates = [
        dataset_dir,
        dataset_dir / "mage_cache" / "real16",
        dataset_dir / "real16",
    ]
    for candidate in candidates:
        if (candidate / "run.json").exists() and (candidate / "manifest.jsonl").exists():
            return candidate
    raise FileNotFoundError(f"could not locate mage_cache/real16 under {dataset_dir}")
...
def _model_call(model: RealMageAlbedoNet, batch: dict[str, torch.Tensor]) -> torch.Tensor:
    return model(batch["hidden"], batch["sampler"], batch["auxiliary"], batch["source"])


@torch.no_grad()
def _update_ema(ema: RealMageAlbedoNet, model: RealMageAlbedoNet, decay: float) -> None:
...
- Actual Mage cache images: {summary['train_images']} train / {summary['validation_images']} validation
- Projected hidden channels: {summary['channels']['hidden']}
- Sampler channels: {summary['channels']['sampler']}
- Auxiliary channels: {summary['channels']['auxiliary']}
...
The training set contains only twelve real aerial-image pairs. This checkpoint is
a real Mage-conditioned model, but it is an initial narrow-domain model rather
than a general-purpose albedo estimator. More paired scenes are required for broad
generalization.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train the real full-resolution Mage albedo model.")
    parser.add_argument(
        "--hf-dataset",
        default="ApacheOne/voir-mage-flow-edit-turbo-cache16",
        help="Public Hugging Face dataset repo containing mage_cache/real16.",
    )
    parser.add_argument("--dataset-dir", default="/content/voir_mage_cache16")
    parser.add_argument("--output-dir", default="/content/voir_real_mage_albedo_v1")
    parser.add_argument("--epochs", type=int, default=50)
    parser.add_argument("--batch-size", type=int, default=2)
    parser.add_argument("--repeats", type=int, default=4)
    parser.add_argument("--patch-grid", default="16,24")
    ...
    parser.add_argument(
        "--upload-repo",
        default=None,
        help="Optional Hugging Face model repo, for example ApacheOne/voir-real-mage-albedo-v1.",
    )
    args = parser.parse_args()

    _seed_everything(args.seed)
    if not torch.cuda.is_available():
        raise RuntimeError("This fast real-model training run requires a CUDA GPU")
    ...
    if not (dataset_dir / "mage_cache" / "real16" / "run.json").exists():
    ...
    if abs(scale_h - scale_w) > 1e-6:
        raise ValueError(f"nonuniform cache scale is unsupported: {scale_h} x {scale_w}")
    upsample_stages = round(np.log2(scale_h))
    if 2**upsample_stages != round(scale_h):
        raise ValueError(f"cache scale must be a power of two, got {scale_h}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if should_validate:
            ...
            improved = best is None or validation["mae"] < best["validation"]["mae"]
            if improved:
                best = dict(row)
    ...
    if best_state is None or best is None:
        raise RuntimeError("training did not produce a held-out checkpoint")
    ...
    checkpoint_path = output_dir / "voir_real_mage_albedo_v1.pt"
    ...
    if args.upload_repo:
        ...
        if not token:
            raise RuntimeError("--upload-repo requires HF_TOKEN in the environment")
        ...
        commit = api.upload_folder(
            ...
            commit_message="Train real Mage-conditioned albedo model",
        )
